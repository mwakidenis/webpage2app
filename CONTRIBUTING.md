## How to contribute to webpg2app

**Welcome to create [pull requests](https://github.com/mwakidenis/webpg2app/compare/) for bugfix, new component, doc, example, suggestion and anything.**
...
git clone https://github.com/mwakidenis/webpg2app.git
cd webpg2app
...
- **`cargo` command not found after installation**: webpg2app CLI now reloads the Rust environment automatically, but if the issue persists reopen your terminal or run `source ~/.cargo/env` (macOS/Linux) / `call %USERPROFILE%\.cargo\env` (Windows) before retrying
