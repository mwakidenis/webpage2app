...
export async function mergeConfig(
  ...
  // Ensure .webpg2app directory exists and copy source templates if needed
  const srcTauriDir = path.join(npmDirectory, 'src-tauri');
  await fsExtra.ensureDir(tauriConfigDirectory);

  // Copy source config files to .webpg2app directory (as templates)
  const sourceFiles = [
    'tauri.conf.json',
  ...
  const linuxBinaryName = `webpg2app-${generateLinuxPackageName(name)}`;
  tauriConf.mainBinaryName =
    platform === 'linux'
      ? linuxBinaryName
      : `webpg2app-${generateIdentifierSafeName(name)}`;
  ...
  if (platform === 'linux') {
    ...
    const desktopFileName = `com.webpg2app.${linuxName}.desktop`;
...
`;
  ...
  const pakeConfigPath = path.join(tauriConfigDirectory, 'webpg2app.json');
