...
function resolveAppName(name: string, platform: NodeJS.Platform): string {
  const domain = getDomain(name) || 'webpg2app';
...
function resolveLocalAppName(
  ...
  const baseName = path.parse(filePath).name || 'webpg2app-app';
  if (platform === 'linux') {
    return generateLinuxPackageName(baseName) || 'webpg2app-app';
  }
  const normalized = baseName
    ...
    .replace(/\s+/g, ' ')
    .trim();
  return normalized || 'webpg2app-app';
}
